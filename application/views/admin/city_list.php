...
<!--			</div>-->
			<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
				<div style="width:100%;">				
					<div style="float:left;margin-right:8px;">
						...
						<a href="add_page"><span class="btn btn-xs btn-pink">添加开通城市</span></a>
						<?php }?>
					</div>
					<div style="float:right;">		
						<form action="<?=$this->go_url;?>" method="post">	
							<div style="float:left;margin-right:10px;">
								<select style="width:100px;" id="parent_id" name="parent_id">
									<option value="">开通省份</option>
									<?php foreach($parent_list as $key => $value) { ?>
										<option value="<?=$value['region_id'];?>" <?php if($parent_id == $value['region_id']) echo 'selected="selected;"';?>><?=$value['region_name'];?></option>
									<?php }?>
								</select>
								<input type="text" id="search_field" name="search_field" placeholder="输入开通城市"  value="<?=$search_field;?>">
							</div>								
	...
	jQuery(function($) {
		...
		var oTable1 = $('#sample-table-2').dataTable({ 
            ...
            "sAjaxSource": admin_path+"/city/ajax_city_list?search_field="+search_field+"&parent_id="+parent_id, //给服务器发请求的url
            "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
	       	    {"mData": 'id'},
                {"mData": 'region_name'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
                {"mData": 'region_name_city'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
                {"mData": 'operate'},
            ],
            "aoColumnDefs": [//和aoColums类似，但他可以给指定列附近爱属性
               {"bSortable": false, "aTargets": [2,3]},//（从0开始算） 不能排序
        	]
		});
	});
