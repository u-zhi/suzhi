	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			var oTable1 = $('#sample-table-2').dataTable({
	            ...
	            "sAjaxSource": admin_path+"/balance/ajax_income_list?&user_id="+user_id, //给服务器发请求的url
	            "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
		       	    {"mData": 'id'},
					{"mData": 'create_time'},
	                {"mData": 'amount'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
	                {"mData": 'comment'},
	            ],
	            "aoColumnDefs": [//和aoColums类似，但他可以给指定列附近爱属性
	               {"bSortable": false, "aTargets": [3]},//（从0开始算） 不能排序
	        	]
			});
		});
