			...
			<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
				<div style="width:100%;">				
					<div style="float:left;margin-right:8px;">		
						...
						<a href="add_page"><span class="btn btn-xs btn-pink">添加文章</span></a>	
						<?php }?>
					</div>
					<div style="float:right;">		
						<form action="<?=$this->go_url;?>" method="post">
							<div style="float:left;margin-right:10px;">
								<input type="text" id="search_field" name="search_field" placeholder="输入文章名称搜索"  value="<?=$search_field;?>">
							</div>								
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			var oTable1 = $('#sample-table-2').dataTable({ 
	            ...
	            "sAjaxSource": admin_path+"/news/ajax_news_list?search_field="+search_field, //给服务器发请求的url
	            "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
	               {"mData": 'check'},
		       	    {"mData": 'id'},
	                {"mData": 'title'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
	                {"mData": 'news_sort'},
	                {"mData": 'update_time'},
	                {"mData": 'operate'},
	            ],
