			...
			<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
				<div style="width:100%;">				
					<div style="float:left;margin-right:8px;">		
						...
						<a href="add_page"><span class="btn btn-xs btn-pink">添加商品</span></a>	
						<?php }?>
					</div>
					<div style="float:right;">		
						<form action="<?=$this->go_url;?>" method="post">	
							<div style="float:left;margin-right:10px;">
								<select style="width:100px;" id="classify_id" name="classify_id">
									<option value="">所有分类</option>
									<?php foreach($classify_list as $key => $value) { ?>
										<option value="<?=$value['id'];?>" <?php if($classify_id == $value['id']) echo 'selected="selected;"';?>><?=$value['name'];?></option>
									<?php }?>
								</select>
								<input type="text" id="search_field" name="search_field" placeholder="输入商品名称搜索"  value="<?=$search_field;?>">
							</div>								
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			var oTable1 = $('#sample-table-2').dataTable({ 
	            ...
	            "sAjaxSource": admin_path+"/goods/ajax_goods_list?search_field="+search_field+"&classify_id="+classify_id, //给服务器发请求的url
	            ...
	            "fnCreatedRow": function(nRow, aData,iDataIndex) {
	                $('td:eq(0)',nRow).html("<span style='line-height:20px;' class='row-details row-details-close' data_id='" + aData.id + "'></span>&nbsp;<span class = 'ser_id'>" + aData.id + "</span>");
	            },
		   ...
		   function fnFormatDetails(nTr, pdataId) {
		       //根据配置Id 异步查询数据
		       $.get("/user_admin/goods/ajax_goods_info/"+ pdataId,function(json) {
			       	var json = eval("("+json+")");
					var sOut = json.str;
