					...
					<form class="form-horizontal" enctype="multipart/form-data" role="form" method="post" action="">
						<div class="visible-md visible-lg hidden-sm hidden-xs btn-group" style='margin: 20px 0 40px 20%;'>
							<div style="float:left;margin-right:30px;">
								<a href="<?=$admin_path;?>/order/edit_pageseseses/<?=$data['id']?>" class='btn-pink2'><span  class="btn2 btn-xs bottom-ky ">招聘信息</span></a>
							</div>					
							<div style="float:left;margin-right:30px;">
								<a href="<?=$admin_path;?>/order/wait_list/<?=$data['id']?>" class='btn-pink3'><span class="btn2 btn-xs  bottom-ky ">简历投递的求职者</span></a>
							</div>							
							<div style="float:left;margin-right:30px;">
								<a href="<?=$admin_path;?>/order/people_list_tock/<?=$data['id']?>" class='btn-pink3'><span class="btn2 btn-xs  bottom-ky ">面试的求职者</span></a>
							</div>						
						</div>
						...
						<!-- <div class="form-group">
							...
							<div class="col-sm-2">
								<input type="text" class="width-100"  value="<?=$data['order_type']?>" readonly="readonly">
							</div>
						</div> -->
...
<!-- 								<a href="/user_admin/jobhunter/show_resume/<?=$data['user_id']?>" class="btn btn-sm btn-yellow">查看简历</a>
								<button type="button" onclick="nopass()" class="btn btn-sm btn-primary">改为正常单</button> -->
							</div>
					  	</div>
					</form>	
				</div>
	...
	<script type="text/javascript" >
		function nopass(status) {
			...
			$.ajax({
				type: "post",
				url:"/user_admin/order/problem_edit",
				data: data,
				dataType: "json",
				success: function(data){
					if(data.status.succeed == 1){
						window.location.href="/user_admin/order/problem_list";
					}else{
