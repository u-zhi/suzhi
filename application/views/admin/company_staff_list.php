...
<!-- 				<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
				...
				<div class="visible-md visible-lg hidden-sm hidden-xs btn-group" style='margin: 124px 0 40px -5%;'>
					...
					<div style="float:left;margin-right:30px;">
						<a href="<?=$admin_path;?>/firm/company_staff/<?=$data['id']?>" class='btn-pink2'><span class="btn2 btn-xs  bottom-ky ">企业成员</span></a>
					</div>
					<div style="float:left;margin-right:30px;">
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			var oTable1 = $('#sample-table-2').dataTable({
	            ...
	            "sAjaxSource": admin_path+"/firm/ajax_company_staff_list?search_field="+search_field+"&firm_id="+firm_id, //给服务器发请求的url
	            "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
		       	    {"mData": 'id'},
	                {"mData": 'company_id'},
	                {"mData": 'department'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
					{"mData": 'name'},
					{"mData": 'mobile'},
					{"mData": 'add_time'},
					{"mData": 'companion'},
	            ...
	            "aoColumnDefs": [//和aoColums类似，但他可以给指定列附近爱属性
	               {"bSortable": false, "aTargets": [6,7]},//（从0开始算） 不能排序
	        	]
			});
		});
