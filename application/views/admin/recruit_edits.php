...
<div class="main-content">
		<div class="breadcrumbs" id="breadcrumbs">
			...
			<ul class="breadcrumb">
				...
				<li><a href="<?=$this->recruit_url;?>">订单中心</a></li>
				<li><a href="<?=$this->recruit_url;?>">面试录取列表</a></li>
				<li>查看信息</li>
			</ul>
		</div>	
		<input type="hidden" name="message" id="message" value="<?=$message?>">
		<input type="hidden" name="admin_path" id="admin_path" value="<?=$admin_path?>">
		<input type="hidden" name="path" id="path" value="<?=$this->recruit_url;?>">
			...
			<div class="row">
				<div class="col-xs-12">
					<form class="form-horizontal" enctype="multipart/form-data" role="form" method="post" action="">
						...
						<hr>
						<div class="col-sm-offset-1 col-sm-10" style="font-size: 20px">当前录取之后第<span style="color:red"><?=$data['time_diff']?></span>天</div>
						<div class="page-header"></div>
					 	<div class="form-group">
					   		<div class="col-sm-offset-1 col-sm-10">
								<a href="/user_admin/jobhunter/show_resume/<?=$data['user_id']?>" class="btn btn-sm btn-yellow">查看简历</a>
								<button type="button" onclick="nopass(1)" class="btn btn-sm btn-primary">3-10日自己离职</button>
								<button type="button" onclick="nopass(2)" class="btn btn-sm btn-black">3-10日企业辞退</button>
	...
	<script type="text/javascript" >
		function nopass(status) {
			...
			$.ajax({
				type: "post",
				url:"/user_admin/order/discharge",
				data: data,
				dataType: "json",
				success: function(data){
					if(data.status.succeed == 1){
						window.location.href="/user_admin/order/recruit_list";
					}else{
