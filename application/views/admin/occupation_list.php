			...
			<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
				<div style="width:100%;">				
					<div style="float:left;margin-right:8px;">
						...
						<a href="add_pages"><span class="btn btn-xs btn-pink">添加职位类别</span></a>
						<a href="add_page"><span class="btn btn-xs btn-pink">添加职位名称</span></a>
						<?php }?>
					</div>
							...
							<div style="float:left;margin-right:10px;">
								<select style="width:100px;" id="parent_id" name="parent_id">
									...
									<?php foreach($parent_list as $key => $value) { ?>
										<option value="<?=$value['id'];?>" <?php if($parent_id == $value['id']) echo 'selected="selected;"';?>><?=$value['item'];?></option>
									<?php }?>
								</select>
								<input type="text" id="search_field" name="search_field" placeholder="输入专业名称搜索"  value="<?=$search_field;?>">
							</div>								
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			var oTable1 = $('#sample-table-2').dataTable({ 
	            ...
	            "sAjaxSource": admin_path+"/occupation/ajax_occupation_list?search_field="+search_field+"&parent_id="+parent_id, //给服务器发请求的url
	            "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空
		       	    {"mData": 'id'},
	                {"mData": 'first_name'}, //mData 表示发请求时候本列的列明，返回的数据中相同下标名字的数据会填充到这一列
	                {"mData": 'item'},
	                {"mData": 'operate'},
	            ],
	            "aoColumnDefs": [//和aoColums类似，但他可以给指定列附近爱属性
	               {"bSortable": false, "aTargets": [1,2,3]},//（从0开始算） 不能排序
	        	]
			});
		});
