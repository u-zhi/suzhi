 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Channel extends PC_Controller {	
	public function __construct() {
		parent::__construct();
		$this->load->model('purchase_model');
		$this->go_url = $this->data['admin_path']."/channel/channel_list";
	...
	public function ajax_channel_list() {
		...
		if($this->data['count']) {
			...
			$type_list = array('','开户','采购','升级');
			$this->load->model('admin_model');
			$company_where['id'] = 1;
			$admin_info = array();
			$admin_info = $this->admin_model->checkAdmin($company_where);
			foreach($purchase_list as $key => &$value) {
				$look_url = $this->edit_url('channel','look_page',$value['id'],'查看','btn-success');	
				$edit_url = '';
				if($value['type'] == 2 && $value['order_status'] == 2) {
					$edit_url = $this->edit_url('channel','edit_page',$value['id']);				
				}
				$value['order_status'] = $order_list[$value['order_status']];
				$value['type'] = $type_list[$value['type']];
	...
	public function export_excel() {
		...
		$export_list = array();
		$export_list = $this->purchase_model->getPurchaseAll(array(),'purchase.*,user.user_name,user.phone,temp.user_name as parent_name,temp.phone as parent_phone');
		$order_list = array('','待付款','待审核','已完成','已取消');
		$type_list = array('','开户','采购','升级');
		$this->load->model('admin_model');
		$company_where['id'] = 1;
		...
		foreach ($export_list as $key => &$value) {
			$order_export[] = array(
					...
					$value['total_price'],$value['type'],$value['order_status']
			);
		}
		$arr_name = array('id','订单号','采购人','采购人电话','上级姓名','上级手机号','订单总价','订单类型','订单状态');
		export_data_excel($arr_name, $order_export, '渠道采购单列表');
	}
}
?>