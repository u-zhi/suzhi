 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Pickup extends PC_Controller {	
	public function __construct() {
		parent::__construct();
		$this->load->model('pickup_model');
		$this->go_url = $this->data['admin_path']."/pickup/pickup_list";
	...
	public function pickup_list() {
		...
		$order_list = array('全部状态','待付款','待发货','待收货','已完成','已取消');
		$this->assign('order_list',$order_list);
		$this->display('pickup_list');
	}
	 ...
	 * @since	2017/01/13 Ver 1.0
	 */
	public function ajax_pickup_list() {
		$data = $_GET;
		...
		$search = $data ['sSearch'];
		$where['concat(moyoo_pickup.id) like'] = '%'.trim($search).'%';	
		if($data['search_field']) {
			$where['concat(moyoo_pickup.order_id) like'] = '%'.trim($data['search_field']).'%';
		}
		if($data['order_status']) {
			$where['pickup.order_status'] = $data['order_status'];
		}
		$this->data['count'] = $this->pickup_model->getCount($where);
		$aaData = array();
		if($this->data['count']) {
			$order_by = $sort_th." ".$sort_type;
			$pickup_list = $this->pickup_model->getPickupList($where,$length,$start,$order_by);
			$order_list = array('','待付款','待发货','待收货','已完成','已取消');
			foreach($pickup_list as $key => &$value) {
				$value['total_price'] = $value['total_price'] + $value['total_dc_price'];
				$look_url = $this->edit_url('pickup','look_page',$value['id'],'查看','btn-success');
				$edit_url = '';
				if($value['order_status'] < 4) {
					$edit_url = $this->edit_url('pickup','edit_page',$value['id']);
				}							
	...
	public function look_page() {
		...
		if(is_numeric($id)) {
			$where['pickup.id'] = $id;
			$pickup_info = $this->pickup_model->checkPickup($where,'pickup.*,user.user_name,user.phone as user_phone,user.agent_level');				
			if($pickup_info) {
				$level_list = array('','一级代理','二级代理','三级代理','四级代理','五级代理');
				$order_list = array('','待付款','待发货','待收货','已完成','已取消');
				$pickup_info['agent_level'] = $level_list[$pickup_info['agent_level']];
				$pickup_info['order_status'] = $order_list[$pickup_info['order_status']];
				/**获取订单的商品列表**/
				$this->load->model('pickup_details_model');
				$goods_where['appoint_id'] = $pickup_info['id'];
				$goods_list = $this->pickup_details_model->getPickupAll($goods_where);				
			}		
		}
		$this->check_rational($pickup_info);
		$this->assign('data',$pickup_info);
		$this->assign('goods_list',$goods_list);
		$this->display('pickup_look');
	}
	...
	public function edit_page() {
		...
		if(is_numeric($id)) {
			...
			if($pickup_info) {
				...
				$order_list = array('','待付款','待发货','待收货','已完成','已取消');
				$pickup_info['agent_level'] = $level_list[$pickup_info['agent_level']];
				$pickup_info['order'] = $order_list[$pickup_info['order_status']];
				/**获取订单的商品列表**/
				$this->load->model('pickup_details_model');
				$goods_where['appoint_id'] = $pickup_info['id'];
				$goods_list = $this->pickup_details_model->getPickupAll($goods_where);				
