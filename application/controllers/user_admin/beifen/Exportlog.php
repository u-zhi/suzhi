 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Exportlog extends PC_Controller {		
	public function __construct() {
		parent::__construct();
		$this->load->model('operation_log_model');
		$this->go_url = $this->data['admin_path']."/exportlog/exportlog_list";
	 ...
	 */
	public function exportlog_list() {
	 ...
	 */
	public function ajax_exportlog_list() {
		$data = $_GET;
		...
		if($this->data['count']) {			
			...
			foreach($log_list as $key => &$value) {	
				...
				$value['message'] = '导出'.$value['message'];
			}
			$aaData = $log_list;
		}
