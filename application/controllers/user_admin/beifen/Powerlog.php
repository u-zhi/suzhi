 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Powerlog extends PC_Controller {		
	public function __construct() {
		parent::__construct();
		$this->load->model('power_log_model');
		$this->go_url = $this->data['admin_path']."/powerlog/powerlog_list";
	 ...
	 * @since	2016/12/21 Ver 1.0
	 */
	public function ajax_powerlog_list() {
		$data = $_GET;
		...
		$search = $data ['sSearch'];
		$where['concat(moyoo_power_log.id) like'] = '%'.trim($search).'%';
		if($data['admin_name']) {
			$where['concat(moyoo_power_log.login_name) like'] = '%'.trim($data['admin_name']).'%';;
		}
		if($data['start_time']) {
			$where['create_time >=']  =  strtotime($data['start_time']);
			$where['create_time <']  =  strtotime($data['start_time']) + 86400;
		}
		$this->data['count'] = $this->power_log_model->getCount($where);
		$aaData = array();
		if($this->data['count']) {			
			$order_by = $sort_th." ".$sort_type;
			$log_list = $this->power_log_model->getPowerList($where,$length,$start,$order_by);
