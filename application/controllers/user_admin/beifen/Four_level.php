 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Four_level extends PC_Controller {		
	public function __construct() {
		parent::__construct();
		$this->load->model('user_model');
		$this->go_url = $this->data['admin_path']."/four_level/four_list";
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function ajax_four_list() {
		$data = $_GET;
		...
		$where['moyoo_user.agent_level'] = 4; //四级代理
		...
		if($data['message_id'] && $data['message'] == 300) {
			$where['moyoo_user.three_id'] = $data['message_id'];
		}
		...
		if($this->data['count']) {			
			...
			foreach($user_list as $key => &$value) {
				$edit_url = $this->edit_url('four_level','edit_page',$value['id']);	
				$look_url = $this->edit_url('four_level','look_page',$value['id'],'查看','btn-pink');
				if($value['is_status'] == 1) {
					$up_url = $this->delete_url('/four_level/agree_four_level',$value['id'],'冻结','btn-success');				
				}else {
					$up_url = $this->delete_url('/four_level/fail_four_level',$value['id'],'解冻','btn-light');
				}
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function agree_four_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function fail_four_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	...
	public function edit_page() {
		...
		if(is_numeric($id)) {		
			...
			if($user_info && $user_info['agent_level'] != 4) {
				$user_info = null;
			}else {
				$user_info['level'] = '四级代理';
			}
		}
		$this->check_rational($user_info);
		$this->assign('data', $user_info);
		$this->display('four_edit');
	}
	...
	public function look_page() {
		...
		/**获取五级代理个数**/
		$five_count = $this->user_model->getCount(array('user.four_id' => $id,'user.agent_level' => '5','user.parent_status' => '3','user.invite_status' => '3'));
		$this->data['data'] = $user_info;
		$this->data['amdin_info'] = $amdin_info;
		$this->data['five_count'] = $five_count;
		$this->display('four_look');
	}
	...
	public function export_excel() {
		...
		export_data_excel($arr_name, $order_export, '四级代理列表');
	}
}
?>