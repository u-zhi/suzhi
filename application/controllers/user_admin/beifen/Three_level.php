 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Three_level extends PC_Controller {		
	public function __construct() {
		parent::__construct();
		$this->load->model('user_model');
		$this->go_url = $this->data['admin_path']."/three_level/three_list";
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function ajax_three_list() {
		$data = $_GET;
		...
		$where['moyoo_user.agent_level'] = 3; //三级代理
		...
		if($data['message_id'] && $data['message'] == 200) {
			$where['moyoo_user.two_id'] = $data['message_id'];
		}
		...
		if($this->data['count']) {			
			...
			foreach($user_list as $key => &$value) {
				$edit_url = $this->edit_url('three_level','edit_page',$value['id']);	
				$look_url = $this->edit_url('three_level','look_page',$value['id'],'查看','btn-pink');
				if($value['is_status'] == 1) {
					$up_url = $this->delete_url('/three_level/agree_three_level',$value['id'],'冻结','btn-success');				
				}else {
					$up_url = $this->delete_url('/three_level/fail_three_level',$value['id'],'解冻','btn-light');
				}
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function agree_three_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	 ...
	 * @since	2016/12/28 Ver 1.0
	 */
	public function fail_three_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	...
	public function edit_page() {
		...
		if(is_numeric($id)) {		
			...
			if($user_info && $user_info['agent_level'] != 3) {
				$user_info = null;
			}else {
				$user_info['level'] = '三级代理';
			}
		}
		$this->check_rational($user_info);
		$this->assign('data', $user_info);
		$this->display('three_edit');
	}
	...
	public function look_page() {
		...
		/**获取四级代理个数**/
		$four_count = $this->user_model->getCount(array('user.three_id' => $id,'user.agent_level' => '4','user.parent_status' => '3','user.invite_status' => '3'));	
		/**获取五级代理个数**/
		$five_count = $this->user_model->getCount(array('user.three_id' => $id,'user.agent_level' => '5','user.parent_status' => '3','user.invite_status' => '3'));
		$this->data['data'] = $user_info;
		$this->data['amdin_info'] = $amdin_info;
		$this->data['four_count'] = $four_count;
		$this->data['five_count'] = $five_count;
		$this->display('three_look');
	}
	...
	public function export_excel() {
		...
		export_data_excel($arr_name, $order_export, '三级代理列表');
	}
}
?>