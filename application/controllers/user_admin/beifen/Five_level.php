 ...
 * ━━━━━━感觉萌萌哒━━━━━━
 */
class Five_level extends PC_Controller {		
	public function __construct() {
		parent::__construct();
		$this->load->model('user_model');
		$this->go_url = $this->data['admin_path']."/five_level/five_list";
	 ...
	 */
	public function ajax_five_list() {
		$data = $_GET;
		...
		$where['moyoo_user.agent_level'] = 5; //五级代理
		...
		if($data['message_id'] && $data['message'] == 400) {
			$where['moyoo_user.four_id'] = $data['message_id'];
		}
		...
		if($this->data['count']) {			
			...
			foreach($user_list as $key => &$value) {
				$edit_url = $this->edit_url('five_level','edit_page',$value['id']);	
				$look_url = $this->edit_url('five_level','look_page',$value['id'],'查看','btn-pink');
				if($value['is_status'] == 1) {
					$up_url = $this->delete_url('/five_level/agree_five_level',$value['id'],'冻结','btn-success');				
				}else {
					$up_url = $this->delete_url('/five_level/fail_five_level',$value['id'],'解冻','btn-light');
				}
	 ...
	 * @since	2016/12/30 Ver 1.0
	 */
	public function agree_five_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	 ...
	 * @since	2016/12/30 Ver 1.0
	 */
	public function fail_five_level() {
		$data = $_POST;
		$where['id'] = $data['id'];
	...
	public function edit_page() {
		...
		if(is_numeric($id)) {		
			...
			if($user_info && $user_info['agent_level'] != 5) {
				$user_info = null;
			}else {
				$user_info['level'] = '五级代理';
			}
		}
	...
	public function export_excel() {
		...
		export_data_excel($arr_name, $order_export, '五级代理列表');
	}
}
?>