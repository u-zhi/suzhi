<?php
//全职任务
class Fulltime extends PC_Controller {
	public function __construct() {
		...
		$this->go_url = $this->data['admin_path']."/fulltime/fulltime_list";
		$this->data['authority'] = $this->authority;
	}
	
	//全职任务列表
	public function fulltime_list() {
		if($this->uri->segment(4)) {
			$this->data['message'] = $this->uri->segment(4);
		}
		$this->data['search_field'] =  empty($_POST) ? '' : $_POST['search_field'];
		$this->display('fulltime_list');
	}
	//ajax获取管理员数据加载到列表
	public function ajax_fulltime_list() {
		...
		if($this->data['count']) {
			...
			foreach($admin_list as $key => &$value) {
				...
				if($value['is_off_shelved'] == 0){
					...
					$up_url = $this->delete_url('/fulltime/fail_up',$value['id'],'下架','btn-light');
				}else{
					$value['is_off_shelved'] = '下架';
					$up_url = $this->delete_url('/fulltime/agree_up',$value['id'],'上架','btn-success');
				}
				$edit_url = $this->edit_url('fulltime','edit_page',$value['id']);
				$del_url = $this->delete_url('/fulltime/delete',$value['id'],'删除','btn-purple');
