<?php
//兼职任务
class Plurality extends PC_Controller {
	public function __construct() {
		...
		$this->go_url = $this->data['admin_path']."/plurality/plurality_list";
		$this->data['authority'] = $this->authority;
	}

	//全职任务列表
	public function plurality_list() {
		if($this->uri->segment(4)) {
			$this->data['message'] = $this->uri->segment(4);
		}
		$this->data['search_field'] =  empty($_POST) ? '' : $_POST['search_field'];
		$this->display('plurality_list');
	}
	//ajax获取管理员数据加载到列表
	public function ajax_plurality_list() {
		...
		if($this->data['count']) {
			...
			foreach($admin_list as $key => &$value) {
				...
				if($value['is_off_shelved'] == 0){
					...
					$up_url = $this->delete_url('/plurality/fail_up',$value['id'],'下架','btn-light');
				}else{
					$value['is_off_shelved'] = '下架';
					$up_url = $this->delete_url('/plurality/agree_up',$value['id'],'上架','btn-success');
				}
				$edit_url = $this->edit_url('plurality','edit_page',$value['id']);
				$del_url = $this->delete_url('/plurality/delete',$value['id'],'删除','btn-purple');
