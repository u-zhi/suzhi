...
class Assess_model extends HS_Model
{
    /*
    *
    * 以下为微信端调用
    ...
    * */
    //获取结果集
    public function getWapAssessList($where = array(), $limit = '20', $offset = '0', $order_by = 'id desc', $fields = "*")
    {
        return $this->select($fields)->where($where)->limit($limit, $offset)->order_by($order_by)->find_all();
    }

    //获取所有结果集
    public function getWapAssessAll($where = array(), $fields = "*", $order_by = 'id desc')
    {
    ...
    //获取单个
    public function checkWapAssess($where, $fields = "*")
    {
        return $this->select($fields)->where($where)->find();
    }

    //删除
    public function deleteWapAssess($where)
    {
    ...
    //添加
    public function addWapAssess($data)
    {
        if ($this->add($data)) {
    ...
    //编辑
    public function editWapAssess($where, $data)
    {
