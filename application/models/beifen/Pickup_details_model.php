...
class Pickup_details_model extends HS_Model {
     ...
     * */
    //获取结果集
    public function getWapPickup_detailsList($where=array(), $limit='20', $offset='0', $order_by='id desc',$fields="*",$user_join=false)
    {
        if($user_join) {
            return $this->select($fields)
                ->join('goods','goods.id = pickup_details.goods_id','left')
    ...
    //获取所有结果集
    public function getWapPickup_detailsAll($where=array(),$fields="*",$order_by='id desc')
    {
        return $this->select($fields)->where($where)->order_by($order_by)->find_all();
    }
    ...
    //获取单个
    public function checkWapPickup_details($where,$fields="*",$user_join=false)
    {
        if($user_join) {
            return $this->select($fields)
                ->join('goods','goods.id = pickup_details.goods_id','left')
    ...
    //删除
    public function deleteWapPickup_details($where)
    {
    ...
    //添加
    public function addWapPickup_details($data)
    {
        if ($this->add($data))
        {
    ...
    //编辑
    public function editWapPickup_details($where,$data)
    {
    ...
    public function addWapId($where,$col,$value){
        ...
        return $this->db->update('pickup_details');
    }
