...
class User_feedback_model extends HS_Model {
    //获取结果集
    public function getFeedbackList($where=array(), $limit='20', $offset='0', $order_by='id desc',$fields="*")
    {
    	return $this->select($fields)->where($where)->limit($limit, $offset)->order_by($order_by)->find_all();
    }
    //获取所有结果集
    public function getFeedbackAll($where=array(),$fields="*",$order_by='id desc',$fields="*")
    {
    	return $this->select($fields)->where($where)->order_by($order_by)->find_all();
    }   
    //获取一个结果集
    public function getFeedbackOne($where=array(),$fields="*",$order_by='id desc',$fields="*")
    {
    ...
    //获取单个
    public function checkFeedback($where=array(),$fields="*")
    {
