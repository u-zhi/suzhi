...
class Cv_project_experience_model extends HS_Model {
    //获取结果集
    public function getProjectList($where=array(), $limit='20', $offset='0', $order_by='id desc',$fields="*")
    {
    	return $this->select($fields)->where($where)->limit($limit, $offset)->order_by($order_by)->find_all();
    }
    //获取所有结果集
    public function getProjectAll($where=array(),$fields="*",$order_by='id desc')
    {
    	return $this->select($fields)->where($where)->order_by($order_by)->find_all();
    }   
    //获取一个结果集
    public function getProjectOne($where=array(),$fields="*",$order_by='id desc',$fields="*")
    {
    ...
    //获取单个
    public function checkProject($where=array(),$fields="*")
    {
    ...
    //编辑
    public function editProject($where,$data)
    {
