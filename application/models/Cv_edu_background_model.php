...
class Cv_edu_background_model extends HS_Model {
    //获取结果集
    public function getEduList($where=array(), $limit='20', $offset='0', $order_by='id desc',$fields="*")
    {
    	return $this->select($fields)->where($where)->limit($limit, $offset)->order_by($order_by)->find_all();
    }
    //获取所有结果集
    public function getEduAll($where=array(),$fields="*",$order_by='id desc')
    {
    	return $this->select($fields)->where($where)->order_by($order_by)->find_all();
    }   
    //获取一个结果集
    public function getEduOne($where=array(),$fields="*",$order_by='id desc',$fields="*")
    {
    ...
    //获取单个
    public function checkEdu($where=array(),$fields="*")
    {
    ...
    //删除
    public function deleteEdu($where) 
    {
    ...
    //编辑
    public function editEdu($where,$data)
    {
