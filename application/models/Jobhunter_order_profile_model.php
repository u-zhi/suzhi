...
class Jobhunter_order_profile_model extends HS_Model {
    //获取结果集
...
//                ->join('headhunter_order_profile','headhunter_order_profile.id = jobhunter_order_profile.headhunter_task_id','left')
                ...
                ->join('user_profile','user_profile.id = jobhunter_order_profile.user_id','left')
    ...
    public function getOrderLists($where=array(), $limit='20', $offset='0', $order_by='id desc',$fields="*",$join=false)
    {
        if($join){
            return $this->select($fields)
                ->join('task_profile','task_profile.id = jobhunter_order_profile.task_id','left')
    ...
    //获取所有结果集
    public function getOrderAll($where=array(),$fields="*",$order_by='id desc')
    {
